        ...
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	eventBusService.subscribe(controllerName, controllerName+'.send', function(event, ojb) {
	            	var m2 = {
  	                  url:"aps/content/page/xxgg/add/config.json",
  	                  contentName:"modal",
  	                  size:"modal-lg",
	                  text:"发布公告",
	                  icon:"open-file"
                	...
                	if(values.length < 1){
                		...
                		$httpService.post(config.updateStatusURL, {"PK_CLASSNEWS":values[0]}).success(function(data) {
                			if(data.code=="0000"){
                        		...
                        		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"公告操作成功"});
                        		
                			}
        	            }).error(function(data) {
        	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"公告操作出错"});
                        });
                	}
	            });
	                	...
	                	if(values.length < 1){
	                		...
	                		$httpService.post(config.deleteURL, {"PK_CLASSNEWS":values[0]}).success(function(data) {
	                			if(data.code=="0000"){
	                				$scope.find();
	                    			eventBusService.publish(controllerName,'appPart.data.reload', {"scope":"site"});
	                        		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"公告删除成功"});
	                        		
	                			}
	        	            }).error(function(data) {
	        	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"公告删除出错"});
	                        });
	                	}
