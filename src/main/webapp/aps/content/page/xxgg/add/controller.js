            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	eventBusService.subscribe(controllerName, controllerName+'.save', function(event, btn) {
            		...
            		$httpService.post(config.addURL, $scope.form).success(function(data) {
            			//if(data.code=="0000"){
	                		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增公告成功"});
	                		eventBusService.publish(controllerName,'appPart.data.reload', {"scope":"bjyd"});
	                		eventBusService.publish(controllerName,'appPart.load.modal.close', {contentName:"modal"});
            			//}else{
            			//	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增公告失败"});
            			//}
    	            }).error(function(data) {
    	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增公告出错"});
                    });
