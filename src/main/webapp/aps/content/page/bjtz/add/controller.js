        ...
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	eventBusService.subscribe(controllerName, controllerName+'.save', function(event, btn) {
            		...
            		$scope.form.CONTENT2 = o_Editor.getText();
            		$scope.form.NEWS_CONTENT = o_Editor.getHTML();
...
//            			if(data.code=="0000"){
	                		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增通知成功"});
	                		eventBusService.publish(controllerName,'appPart.data.reload', {"scope":"bjyd"});
//	                		eventBusService.publish(controllerName,'appPart.load.modal.close', {contentName:"modal"});
//            			}else{
//            				eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增通知失败"});
//            			}
    	            }).error(function(data) {
    	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"新增通知出错"});
                    });
