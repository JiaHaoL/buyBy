        ...
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	$scope.find=function(){
            		...
            		$httpService.post(config.findDeviceManageinfoURL, $scope.form).success(function(data) {
 	         ...
 	         //接收刷新事件
                ...
                eventBusService.subscribe(controllerName, controllerName+'.delete', function(event, ojb) {
                    ...
                    var values = [];
	              	...
	              	if(values.length < 1){
		              	...
		              	if(confirm("是否确认删除！")){
		              		$httpService.post(config.deleteDeviceinfoURL,{"UNIT_DEVICE_PK":values[0]}).success(function(data) {
	                    		$scope.select();
		                     }).error(function(data) {
                ...
                eventBusService.subscribe(controllerName, controllerName+'.add', function(event, ojb) {
                		var m2 = {
    						"url" : "aps/content/api/deviceManage/add/config.json",
    						text:"新增设备",
    						size:"modal-lg",
    						"contentName" : "modal"
    					}
                ...
                eventBusService.subscribe(controllerName, controllerName+'.update', function(event, ojb) {
             	  var values = [];
 	               ...
 	               var m2 = {
 							"url" : "aps/content/api/deviceManage/update/config.json?pk="+values[0],
 							text:"修改设备",
 							size:"modal-lg",
 							"contentName" : "modal"
 						}
