            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
                ...
                eventBusService.subscribe(controllerName, controllerName+'.add', function(event, ojb) {
                	var values = [];
	              	...
	              	if(values.length < 1){
	              		...
	              		var m2 = {
	    						url : "aps/content/api/appSet/classSet/add/config.json?pk="+values[0],
	    						text:"班级座右铭",
	    						size:"modal-lg",
	    						contentName : "modal"
	    					}
                ...
                eventBusService.subscribe(controllerName, controllerName+'.update', function(event, ojb) {
             	  var values = [];
	              	...
	              	if(values.length < 1){
	 	               ...
	 	               var m2 = {
	 							"url" : "aps/content/api/appSet/classSet/update/config.json?pk="+values[0],
	 							text:"修改班级log",
	 							size:"modal-lg",
	 							"contentName" : "modal"
	 						}
                ...
                eventBusService.subscribe(controllerName, controllerName+'.delete', function(event, ojb) {
                    ...
                    var values = [];
	              	...
	              	if(values.length < 1){
		              	...
		              	if(confirm("是否确认删除！")){
		              		$httpService.post(config.deleteURL,{"CLASS_INFO_PK":values[0]}).success(function(data) {
	                    		$scope.select();
		                     }).error(function(data) {
              ...
              //在线预览图片
	           	 $scope.info = function(pk){            		 	           		
	           		var m2 = {
	                  			  "url":"aps/content/api/appSet/classSet/info/config.json?pk="+pk,
	                  		          text:"查看",
	                  		          size:"modal-lg",
	                  		          "contentName":"modal"
	                 }   
